...
export const useReports = () => {
  const {
    bills, customers, purchases, stock, items,
    openingBalances, setOpeningBalances,
    transactions,
  } = useERPStore(
    useShallow(s => ({
      bills: s.bills, customers: s.customers, purchases: s.purchases,
      stock: s.stock, items: s.items,
      openingBalances: s.openingBalances, setOpeningBalances: s.setOpeningBalances,
  ...
  const [editOB, setEditOB] = useState<{ month: string; cashOB: number; bankOB: number } | null>(null);
  ...
  const saveOB = useCallback(() => {
    if (!editOB) return;
    setOpeningBalances((p) => ({ ...p, [editOB.month]: { cash: editOB.cashOB, bank: editOB.bankOB } }));
  ...
  const monthlyData = useMemo(() =>
    months.map((m) => {
      ...
      const creditIn   = mBills.filter((b) => b.payment === 'Credit').reduce((s, b) => s + b.total, 0);
      ...
      return {
        ...
        cashSales, upiSales, creditIn, total,
        purchaseCost, totalExpenses, netProfit,
        cashCB, bankCB,
  ...
  const dailyData = useMemo(() => {
    const byDate = new Map<string, { cash: number; upi: number; credit: number; count: number }>();
    bills.forEach((b) => {
  ...
  const creditCustomers = useMemo(
    () => [...customers.filter((c) => c.credit)].sort((a, b) => b.outstanding - a.outstanding),
    [customers],
  );
  ...
  return {
    months, bills, purchases, customers,
