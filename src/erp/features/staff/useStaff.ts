...
export const useStaff = () => {
  ...
  const addStaff = useCallback(async () => {
    ...
    if (!form.u.trim())    { showToast('Username is required', 'error'); return; }
    if (form.p.length < 8) { showToast('Password must be at least 8 characters', 'error'); return; }

    const duplicate = staff.find((s) => s.u.toLowerCase() === form.u.toLowerCase());
  ...
  const saveRole = useCallback(() => {
    ...
    setStaff((p) => {
      const updated = p.map((s) => (s.id === editRoleId ? { ...s, role: newRole } : s));
  ...
  const toggleActive = useCallback(
    (id: string) => {
      setStaff((p) => {
        const updated = p.map((s) => (s.id === id ? { ...s, active: !s.active } : s));
  ...
  const saveReset = useCallback(async () => {
    ...
    if (resetPwd.length < 8) { showToast('Password must be at least 8 characters', 'error'); return; }

    const hashed = await hashPassword(resetPwd);  // ← SHA-256 hash

    setStaff((p) => {
      const updated = p.map((s) => (s.id === resetTarget.id ? { ...s, p: hashed } : s));
