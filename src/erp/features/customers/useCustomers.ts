...
export const useCustomers = () => {
  ...
  const openAdd     = useCallback(() => { setForm(EMPTY_FORM); setAddModal(true); }, []);
  const closeAdd    = useCallback(() => { setAddModal(false); setForm(EMPTY_FORM); }, []);
  const openCollect  = useCallback((c: Customer) => { setPayModal(c); setPayAmt(''); }, []);
  const closeCollect = useCallback(() => { setPayModal(null); setPayAmt(''); }, []);
  ...
  const recordPayment = useCallback(() => {
    if (!payModal) return;
    if (!payAmt || +payAmt <= 0) { showToast('Enter a valid amount', 'error'); return; }
    setCustomers(p => {
      const updated = p.map(c => {
        if (c.id !== payModal!.id) return c;
        const next = { ...c, outstanding: Math.max(0, c.outstanding - +payAmt) };
        syncCustomer(next);
        return next;
      });
      return updated;
