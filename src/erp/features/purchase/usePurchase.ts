...
export const usePurchase = () => {
  const { items, stock, purchases, setPurchases, setStock } = useERPStore(
    useShallow(s => ({
      items: s.items, stock: s.stock, purchases: s.purchases,
      setPurchases: s.setPurchases, setStock: s.setStock,
    })),
  );
  ...
  const makeEmptyRows = useCallback((): RowsMap => {
    const r: RowsMap = {};
    activeItems.forEach(i => { r[i.id] = { qty: '', rate: '' }; });
    return r;
  }, [activeItems]);
  ...
  const lines = useMemo((): PurchaseLine[] =>
    activeItems
  ...
  const grandTotal = useMemo(() => lines.reduce((s, l) => s + l.total, 0), [lines]);

  // ── All-time stats ────────────────────────────────────────
  const stats = useMemo(() => ({
    totalOrders: purchases.length,
    totalQty:    purchases.reduce((s, p) => s + p.lines.reduce((ss, l) => ss + l.qty, 0), 0),
  ...
  const recordPurchase = useCallback(() => {
    ...
    lines.forEach(l => {
      setStock(p => ({ ...p, [l.itemId]: { qty: (p[l.itemId]?.qty ?? 0) + l.qty } }));
    });

    resetForm();
    setView('history');
    showToast(`✓ Purchase ${ref} recorded`);
  }, [lines, date, note, grandTotal, purchases, setPurchases, setStock, resetForm, setView, showToast]);
