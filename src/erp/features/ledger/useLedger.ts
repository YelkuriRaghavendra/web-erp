...
export const useLedger = () => {
  ...
  const creditCustomers = useMemo(
    () => customers.filter(c => c.credit),
    [customers]
  );
  const totalOutstanding = useMemo(
    () => creditCustomers.reduce((s, c) => s + c.outstanding, 0),
    [creditCustomers]
  );
  ...
  const openDebit = useCallback(() => {
    setEntryForm({ ...EMPTY_ENTRY, type: 'DEBIT' });
    setEntryModal(true);
  }, []);
  const openCredit = useCallback(() => {
    setEntryForm({ ...EMPTY_ENTRY, type: 'CREDIT' });
    setEntryModal(true);
  }, []);
  ...
  const addEntry = useCallback(() => {
    ...
    closeEntry();
    showToast(
      entryForm.type === 'DEBIT' ? 'Debit entry added' : 'Payment recorded'
    );
  }, [entryForm, selectedId, setCustomers, closeEntry, showToast]);

  const previewBalance = useMemo(() => {
    if (!selectedCustomer || !entryForm.amount || +entryForm.amount <= 0)
      return null;
