...
export const useAccounts = () => {
  ...
  const {
    ...
    const todayDeposits = todayTxns
      .filter(t => t.type === 'CASH_TO_BANK')
      .reduce((s, t) => s + t.amount, 0);
    const todayWithdrawals = todayTxns
      .filter(t => t.type === 'BANK_TO_CASH')
      .reduce((s, t) => s + t.amount, 0);
