...
// ── Bill detail modal ─────────────────────────────────────────
const BillDetailModal = ({ bill, onClose }: { bill: Bill; onClose: () => void }) => {
    ...
    >
      ...
      >
        ...
        <div style={{ background: 'var(--sidebar)', padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: 10, fontWeight: 800, color: 'rgba(255,255,255,.4)', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: 6 }}>Bill Details</div>
            <div style={{ fontSize: 22, fontWeight: 900, fontFamily: "'JetBrains Mono',monospace", color: '#ff9a5c', letterSpacing: '-.01em' }}>{bill.id}</div>
            <div style={{ display: 'flex', gap: 16, marginTop: 8, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 12, color: 'rgba(255,255,255,.5)' }}>📅 {bill.date}</span>
              <span style={{ fontSize: 12, color: 'rgba(255,255,255,.5)' }}>👤 {bill.customerName || 'Walk-in'}</span>
              <span style={{ fontSize: 12, color: 'rgba(255,255,255,.5)' }}>
                {bill.payment === 'Cash' ? '💵' : bill.payment === 'UPI' ? '🏦' : '📋'} {bill.payment}
              </span>
            </div>
          </div>
          <button
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((line: BillLine, li) => (
              ...
              >
                ...
                <div style={{ padding: '13px 16px', textAlign: 'right', fontFamily: "'JetBrains Mono',monospace", fontWeight: 800, fontSize: 15, color: 'var(--accent)' }}>{line.qty}</div>
                <div style={{ padding: '13px 16px', textAlign: 'right', fontFamily: "'JetBrains Mono',monospace", fontSize: 13, color: 'var(--ink2)', fontWeight: 600 }}>₹{line.price.toLocaleString()}</div>
                <div style={{ padding: '13px 16px', textAlign: 'right', fontFamily: "'JetBrains Mono',monospace", fontWeight: 800, fontSize: 14, color: 'var(--ink)' }}>₹{line.amount.toLocaleString()}</div>
              </div>
            ))
          )}
        </div>
        ...
        {/* Total footer */}
        <div style={{ flexShrink: 0, borderTop: '2px solid var(--accentbd)', background: 'var(--accentbg)', padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--ink2)' }}>
            {bill.lines.length} item{bill.lines.length !== 1 ? 's' : ''} · Total
          </span>
          <span style={{ fontFamily: "'JetBrains Mono',monospace", fontWeight: 900, fontSize: 22, color: 'var(--accent)' }}>
...
type TodaySummary = { count: number; cash: number; upi: number; credit: number };
...
const BillsTable = ({ bills, todaySummary }: { bills: Bill[]; todaySummary: TodaySummary }) => {
  const [search,       setSearch]       = useState('');
  ...
  const SortIcon = ({ col }: { col: SortCol }) => (
    <span style={{ marginLeft: 4, fontSize: 9, opacity: sortCol === col ? 1 : 0.3, color: sortCol === col ? 'var(--accent)' : 'inherit' }}>
    ...
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
          <StatCard icon="🧾" label="Today's Bills"  value={todaySummary.count}                         sub="Bills today"    color="orange"/>
          <StatCard icon="💵" label="Cash Today"     value={`₹${todaySummary.cash.toLocaleString()}`}   sub="Cash collected" color="green"/>
          <StatCard icon="🏦" label="UPI Today"      value={`₹${todaySummary.upi.toLocaleString()}`}    sub="Bank account"   color="blue"/>
          <StatCard icon="📋" label="Credit Today"   value={`₹${todaySummary.credit.toLocaleString()}`} sub="On credit"      color="red"/>
        </div>
        ...
        <div style={{ background: 'var(--canvas)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden', boxShadow: 'var(--shadow2)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: GRID, background: 'var(--bg)', borderBottom: '1px solid var(--border)' }}>
            {COLS.map((col, i) => (
              <div
                key={`h-${i}`}
                ...
                style={{
                  ...
                  color: col.key && sortCol === col.key ? 'var(--accent)' : 'var(--ink3)',
                ...
                onMouseLeave={e => { e.currentTarget.style.color = col.key && sortCol === col.key ? 'var(--accent)' : 'var(--ink3)'; }}
              >
                {col.label}
          ...
          {bills.length === 0 ? (
            <div style={{ padding: '56px 24px', textAlign: 'center', color: 'var(--ink3)' }}>
              ...
              <div style={{ fontWeight: 700 }}>No bills yet — switch to New Entry to create your first bill.</div>
            </div>
          ) : filtered.length === 0 ? (
            <div style={{ padding: '48px 24px', textAlign: 'center', color: 'var(--ink3)' }}>
              <div style={{ fontSize: 32, marginBottom: 10 }}>⊘</div>
              <div style={{ fontWeight: 700, fontSize: 14 }}>No bills match <em>"{search}"</em></div>
            ...
            filtered.map((bill, idx) => (
              <div
                key={bill.id}
                style={{ display: 'grid', gridTemplateColumns: GRID, alignItems: 'center', borderBottom: idx < filtered.length - 1 ? '1px solid var(--border)' : 'none', background: 'var(--canvas)', transition: 'background .15s' }}
                onMouseEnter={e => { e.currentTarget.style.background = 'var(--bg)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'var(--canvas)'; }}
              >
                {/* Bill No */}
                <div style={{ padding: '14px 14px', fontFamily: "'JetBrains Mono',monospace", fontSize: 12, color: 'var(--accent)', fontWeight: 700 }}>{bill.id}</div>
                {/* Date */}
                <div style={{ padding: '14px 14px', fontSize: 13, color: 'var(--ink2)' }}>{bill.date}</div>
                {/* Customer */}
                <div style={{ padding: '14px 14px' }}>
                  {bill.customerName
                    ? <span style={{ fontWeight: 700, fontSize: 13 }}>{bill.customerName}</span>
                ...
                <div style={{ padding: '14px 14px', textAlign: 'right' }}>
                  <span style={{ fontFamily: "'JetBrains Mono',monospace", fontWeight: 800, fontSize: 14, color: 'var(--ink)' }}>₹{bill.total.toLocaleString()}</span>
                </div>
                {/* Payment */}
                <div style={{ padding: '14px 14px' }}>
                  <Badge label={payLabel(bill.payment)} color={payColor(bill.payment)} />
                </div>
...
export const BillingPage = () => {
  const {
    ...
    note, setNote,
    qtys, setQtyField,
    activeItems, stock, customers, bills,
    lines, total, selectedCustomer, payOpts,
    todaySummary,
    createBill, resetForm,
  } = useBilling();
  ...
  return (
    <div className="page-enter" style={{ display: 'flex', flexDirection: 'column', gap: 0, maxWidth: 1300 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--ink)', letterSpacing: '-.02em' }}>Billing</h1>
        ...
        <div style={{ display: 'flex', gap: 8, background: 'var(--canvas)', border: '1px solid var(--border)', borderRadius: 10, padding: 4 }}>
          {([{ id: 'entry', l: '⌨ New Entry' }, { id: 'history', l: '📊 Daily Sales' }] as const).map(t => (
            <button key={t.id} onClick={() => setView(t.id as 'entry' | 'history')}
              style={{
                ...
                cursor: 'pointer', background: view === t.id ? 'var(--accent)' : 'transparent',
                color: view === t.id ? '#fff' : 'var(--ink3)', transition: 'all .15s',
                fontFamily: "'Plus Jakarta Sans',sans-serif",
              }}>
              {t.l}
            </button>
      ...
      {view === 'entry' ? (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 20, alignItems: 'start' }}>
          {/* LEFT: item entry */}
          <div style={{
            display: 'flex', flexDirection: 'column', gap: 0, background: 'var(--canvas)',
            borderRadius: 16, border: '1px solid var(--border)', overflow: 'hidden', boxShadow: 'var(--shadow2)',
          }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', background: 'var(--sidebar)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 800, color: '#fff' }}>Items</div>
                <div style={{ fontSize: 11, color: 'rgba(255,255,255,.4)', marginTop: 2 }}>Enter qty sold — leave blank to skip</div>
              </div>
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                {lines.length > 0 && (
                  <span style={{ background: 'var(--accent)', color: '#fff', borderRadius: 99, fontSize: 11, fontWeight: 800, padding: '3px 10px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 90px 110px 100px', background: 'var(--bg)', borderBottom: '1px solid var(--border)' }}>
              {(['Item / Stock', 'Qty', 'Rate ₹', 'Amount'] as const).map((h, i) => (
            ...
            {activeItems.length === 0 && (
              <div style={{ padding: '48px 24px', textAlign: 'center', color: 'var(--ink3)' }}>
                <div style={{ fontSize: 36, marginBottom: 10 }}>📦</div>
                <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 6 }}>No stock available</div>
                <div style={{ fontSize: 12 }}>Add stock in Item Master before billing</div>
              </div>
            )}
            ...
            {activeItems.map((item, idx) => {
              ...
              const has   = qty > 0;
              return (
                <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '1fr 90px 110px 100px', alignItems: 'center', borderBottom: idx < activeItems.length - 1 ? '1px solid var(--border)' : 'none', background: has ? '#fffbf7' : 'var(--canvas)', transition: 'background .2s' }}>
                  <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ width: 3, height: 32, borderRadius: 99, background: has ? 'var(--accent)' : 'var(--border)', flexShrink: 0 }} />
                    <div>
                      <div style={{ fontSize: 13.5, fontWeight: has ? 700 : 500, color: has ? 'var(--ink)' : 'var(--ink2)' }}>{item.name}</div>
                      <div style={{ fontSize: 11, marginTop: 3, fontWeight: 600, color: 'var(--green)' }}>{avail} in stock</div>
                    </div>
                  </div>
                  <div style={{ padding: '8px 10px' }}>
                    <input type="number" min="0" placeholder="—"
                      value={qtys[item.id]?.qty || ''}
                      onChange={e => setQtyField(item.id, 'qty', e.target.value)}
                      style={{ width: '100%', textAlign: 'center', background: has ? 'var(--canvas)' : 'var(--bg)', border: `2px solid ${has ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 8, padding: '8px 6px', fontSize: 15, fontWeight: 800, color: has ? 'var(--accent)' : 'var(--ink3)', outline: 'none', fontFamily: "'JetBrains Mono',monospace" }} />
                  </div>
                  <div style={{ padding: '12px 16px', textAlign: 'right' }}>
                    <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 13, fontWeight: 700, color: 'var(--ink2)' }}>
                      ₹{rate.toLocaleString()}
                    </span>
                  </div>
                  <div style={{ padding: '12px 16px', textAlign: 'right' }}>
                    <span style={{ fontFamily: "'JetBrains Mono',monospace", fontWeight: 800, fontSize: 14, color: has ? 'var(--ink)' : 'var(--ink3)' }}>
          ...
          <div style={{ position: 'sticky', top: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ background: 'var(--canvas)', borderRadius: 16, border: '1px solid var(--border)', overflow: 'hidden', boxShadow: 'var(--shadow2)' }}>
              ...
              <div style={{ background: 'var(--sidebar)', padding: '16px 20px' }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: 'rgba(255,255,255,.4)', textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 4 }}>Live Bill</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                  <span style={{ fontSize: 28, fontWeight: 900, color: total > 0 ? '#ff9a5c' : 'rgba(255,255,255,.2)', fontFamily: "'JetBrains Mono',monospace" }}>
                    ₹{total.toLocaleString()}
                  </span>
                  <span style={{ fontSize: 12, color: 'rgba(255,255,255,.3)' }}>{lines.length} item{lines.length !== 1 ? 's' : ''}</span>
                </div>
              </div>
              ...
              <div style={{ maxHeight: 200, overflowY: 'auto' }}>
                {lines.length === 0
                  ? <div style={{ padding: '24px 20px', textAlign: 'center', color: 'var(--ink3)', fontSize: 13 }}>Enter quantities to start</div>
                  : lines.map(l => (
                    <div key={l.itemId} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', borderBottom: '1px solid var(--border)' }}>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 700 }}>{l.itemName}</div>
                        <div style={{ fontSize: 11, color: 'var(--ink3)', marginTop: 1 }}>{l.qty} × ₹{l.price.toLocaleString()}</div>
                      </div>
                      <span style={{ fontFamily: "'JetBrains Mono',monospace", fontWeight: 700, fontSize: 13, color: 'var(--accent)' }}>₹{l.amount.toLocaleString()}</span>
                    </div>
                  ))
                }
              </div>
              ...
              <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12, borderTop: '1px solid var(--border)' }}>
                ...
                <div>
                  <div style={{ fontSize: 10.5, fontWeight: 800, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 5 }}>
                    Customer <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>(optional)</span>
                  </div>
                  <select value={customerId} onChange={e => setCustomerId(e.target.value)} style={inputSt()}>
                    <option value="">— Walk-in —</option>
                    {customers.map(c => (
                      <option key={c.id} value={c.id}>{c.name}{c.credit ? ' (Credit)' : ''}</option>
                    ))}
                  </select>
                ...
                <div>
                  <div style={{ fontSize: 10.5, fontWeight: 800, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 7 }}>Payment</div>
                  <div style={{ display: 'flex', gap: 6 }}>
                    {payOpts.map(o => (
                      <button key={o.v} onClick={() => setPayment(o.v as 'Cash' | 'UPI' | 'Credit')}
                        style={{
                          flex: 1, padding: '8px 4px', borderRadius: 8,
                          border: `2px solid ${payment === o.v ? (o.v === 'Cash' ? 'var(--green)' : o.v === 'UPI' ? 'var(--blue)' : 'var(--red)') : 'var(--border)'}`,
                          background: payment === o.v ? (o.v === 'Cash' ? 'var(--greenbg)' : o.v === 'UPI' ? 'var(--bluebg)' : 'var(--redbg)') : 'var(--bg)',
                          cursor: 'pointer', fontSize: 11, fontWeight: 700,
                          color: payment === o.v ? (o.v === 'Cash' ? 'var(--green)' : o.v === 'UPI' ? 'var(--blue)' : 'var(--red)') : 'var(--ink3)',
                ...
                <div>
                  ...
                  <input type="text" value={note} onChange={e => setNote(e.target.value)} placeholder="Optional remark…" style={inputSt()} />
                </div>
                <button onClick={createBill} disabled={!lines.length}
                  style={{
                    width: '100%', padding: 13, borderRadius: 10, border: 'none',
                    background: lines.length ? 'var(--accent)' : 'var(--border)',
                    ...
                    boxShadow: lines.length ? '0 4px 14px rgba(232,98,10,.35)' : 'none',
                  }}
                  onMouseEnter={e => { if (lines.length) e.currentTarget.style.background = '#c9540a'; }}
                  onMouseLeave={e => { if (lines.length) e.currentTarget.style.background = 'var(--accent)'; }}>
                  {lines.length ? `Create Bill · ₹${total.toLocaleString()}` : 'Enter items to bill'}
                </button>
              </div>
            </div>
            ...
            <div style={{ background: 'var(--canvas)', borderRadius: 12, border: '1px solid var(--border)', padding: '14px 16px', boxShadow: 'var(--shadow)' }}>
              <div style={{ fontSize: 10.5, fontWeight: 800, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 10 }}>Today's Summary</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                {[
                  { l: 'Bills raised', v: todaySummary.count,                              c: 'var(--ink)' },
                  { l: '💵 Cash',      v: `₹${todaySummary.cash.toLocaleString()}`,         c: 'var(--green)' },
                  { l: '🏦 UPI',       v: `₹${todaySummary.upi.toLocaleString()}`,          c: 'var(--blue)' },
                  { l: '📋 Credit',    v: `₹${todaySummary.credit.toLocaleString()}`,       c: 'var(--red)' },
                ].map(r => (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink2)' }}>Total billed</span>
                  <span style={{ fontSize: 15, fontWeight: 900, color: 'var(--accent)', fontFamily: "'JetBrains Mono',monospace" }}>
                    ₹{(todaySummary.cash + todaySummary.upi + todaySummary.credit).toLocaleString()}
                  </span>
                </div>
              </div>
            </div>
