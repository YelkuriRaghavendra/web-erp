...
export const useBilling = () => {
  const { items, stock, customers, bills, setBills, setStock, setCustomers } = useERPStore(
    useShallow(s => ({
      items: s.items, stock: s.stock, customers: s.customers, bills: s.bills,
      setBills: s.setBills, setStock: s.setStock, setCustomers: s.setCustomers,
    })),
  );
  ...
  const makeEmptyQtys = useCallback((): QtysMap => {
    const q: QtysMap = {};
    activeItems.forEach(i => { q[i.id] = { qty: '', rate: String(i.price) }; });
    return q;
  }, [activeItems]);
  ...
  const todaySummary = useMemo(() => ({
    ...
    cash:   todayBills.filter(b => b.payment === 'Cash').reduce((s, b) => s + b.total, 0),
    upi:    todayBills.filter(b => b.payment === 'UPI').reduce((s, b) => s + b.total, 0),
    credit: todayBills.filter(b => b.payment === 'Credit').reduce((s, b) => s + b.total, 0),
  }), [todayBills]);
  ...
  const createBill = useCallback(() => {
    if (!lines.length) { showToast('Enter qty for at least one item', 'error'); return; }
    ...
    // Deduct stock
    lines.forEach(l => {
      setStock(p => ({ ...p, [l.itemId]: { qty: Math.max(0, (p[l.itemId]?.qty ?? 0) - l.qty) } }));
    });
    ...
    if (selectedCustomer && payment === 'Credit') {
      setCustomers(p =>
        p.map(c => {
          ...
          const prevBal    = ledger.length > 0 ? ledger[ledger.length - 1].balance : c.outstanding;
  ...
  }, [lines, date, selectedCustomer, payment, total, note, bills, setBills, setStock, setCustomers, resetForm, setView, showToast]);
  ...
  return {
    ...
    lines, total, selectedCustomer, payOpts, todaySummary,
