...
export const LoginPage = ({ onLogin }: { onLogin: (u: ERPUser) => void }) => {
  ...
  const go = async () => {
    ...
    if (lock && lock.until > Date.now()) {
      ...
      setErr(`Too many failed attempts. Try again in ${Math.ceil((lock.until - Date.now()) / 1000)}s`);
      return;
    }

    // Hash the entered password before comparing with stored hash
    const hashedInput = await hashPassword(password);
    const match = staff.find(s => s.u === key && s.p === hashedInput && s.active);

    if (match) {
      failMap.delete(key);
      onLogin({ u: match.u, role: match.role, name: match.name, createdAt: Date.now() });
    } else {
      ...
      if (until > 0) {
        ...
        setErr(`Invalid credentials${left <= 2 ? ` — ${left} attempt${left !== 1 ? 's' : ''} left` : ''}`);
      }
    }
  };
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', background:'var(--bg)' }}>
      ...
      <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:32 }}>
        <div style={{ width:'100%', maxWidth:380, animation:'fadeUp .35s ease' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:40 }}>
            <div style={{ width:44, height:44, background:'var(--accent)', borderRadius:12,
              display:'flex', alignItems:'center', justifyContent:'center',
              fontSize:24, boxShadow:'0 4px 12px rgba(232,98,10,.3)' }}>🔥</div>
            <div>
              <div style={{ fontSize:20, fontWeight:800, color:'var(--ink)' }}>GasERP</div>
              <div style={{ fontSize:12, color:'var(--ink3)' }}>Gas Agency Management</div>
            </div>
          </div>

          {/* Heading */}
          <div style={{ marginBottom:28 }}>
            <h2 style={{ fontSize:24, fontWeight:800, color:'var(--ink)', marginBottom:6 }}>Welcome back</h2>
            <p  style={{ fontSize:14, color:'var(--ink3)' }}>Sign in to your account to continue</p>
          </div>
          ...
          {noAccounts ? (
            ...
            <div style={{
              ...
              <div style={{ fontSize:14, fontWeight:800, color:'var(--amber)' }}>⚠ No accounts found</div>
              <div style={{ fontSize:12.5, color:'var(--ink2)', lineHeight:1.6 }}>
                Either the database is empty or access is blocked (RLS).<br />
                Check the <strong>browser Console</strong> (F12) for the <code style={{ background:'var(--canvas)', padding:'1px 4px', borderRadius:3 }}>[fetchAllStaff]</code> log line — it shows exactly what Supabase returned.
              </div>
              <div style={{ fontSize:12, color:'var(--ink3)', lineHeight:1.8 }}>
                If it shows <code style={{ background:'var(--canvas)', padding:'1px 4px', borderRadius:3 }}>data: []</code>, run this SQL in Supabase:
              </div>
              <pre style={{
                margin:0, fontSize:11, background:'var(--canvas)', borderRadius:6,
                padding:'10px 12px', color:'var(--ink2)', overflowX:'auto',
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:14 }} onKeyDown={handleKey}>
              ...
              {err && (
                <div style={{
                  background:'var(--redbg)', border:'1px solid var(--redbd)', borderRadius:8,
                  padding:'10px 14px', fontSize:13, color:'var(--red)', fontWeight:600,
                }}>
                  ⚠ {err}
                </div>
              )}
              ...
              {lockSecs > 0 && (
                <div style={{
                  background:'var(--amberbg)', border:'1px solid var(--amberbd)', borderRadius:8,
                  padding:'10px 14px', fontSize:13, color:'var(--amber)', fontWeight:600,
                }}>
      ...
      <div style={{
        ...
        <div style={{ position:'absolute', top:-60, right:-60, width:240, height:240, borderRadius:'50%', background:'rgba(232,98,10,.12)' }}/>
        <div style={{ position:'absolute', bottom:-40, left:-40, width:180, height:180, borderRadius:'50%', background:'rgba(232,98,10,.08)' }}/>
        <div style={{ position:'relative', textAlign:'center' }}>
          <div style={{ fontSize:64, marginBottom:24 }}>⛽</div>
          <div style={{ fontSize:22, fontWeight:800, color:'#fff', marginBottom:12 }}>Smart Gas Agency</div>
          <div style={{ fontSize:14, color:'rgba(255,255,255,.5)', lineHeight:1.6 }}>
            Complete ERP for billing, stock,<br/>customers and monthly reports
          </div>
        </div>
      </div>
    </div>
