...
import {
  fetchAllStaff, fetchAllItems, fetchStock,
  fetchCustomers, fetchBills, fetchPurchases,
  fetchTransactions, fetchOpeningBalances,
} from './core/supabase';
  ...
  <div style={{
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 8 }}>
      <div style={{
        ...
        boxShadow: '0 6px 20px rgba(212,80,0,.4), 0 0 0 1px rgba(255,140,0,.25)',
      }}>
        <svg width="30" height="30" viewBox="0 0 32 32" fill="none">
          <defs>
            <linearGradient id="lsga-bl" x1="16" y1="2" x2="16" y2="30" gradientUnits="userSpaceOnUse">
              <stop offset="0%" stopColor="#FFE566"/>
              <stop offset="50%" stopColor="#FF9200"/>
              <stop offset="100%" stopColor="#D44500"/>
            </linearGradient>
          </defs>
          <path d="M16 2C11 8 7 13 7 18.5C7 24.5 11.1 30 16 30C20.9 30 25 24.5 25 18.5C25 13 21 8 16 2Z" fill="url(#lsga-bl)"/>
      ...
      <div>
        <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--ink)', lineHeight: 1.15, letterSpacing: '-.02em' }}>Laxmi Srinivasa</div>
        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--accent)' }}>Gas Agency</div>
      </div>
    </div>
    <div style={{
    ...
    <div style={{ fontSize: 13, color: 'var(--ink3)', fontWeight: 600 }}>{status}</div>
  </div>
);
...
const GasERPInner = () => {
  ...
  // ── Store setters ─────────────────────────────────────────
  const {
    setStaff, setItems, setStock, setCustomers,
    setBills, setPurchases, setTransactions, setOpeningBalances,
  } = useERPStore(
    useShallow(s => ({
      setStaff:           s.setStaff,
      setItems:           s.setItems,
  ...
  const doBootstrap = useCallback(async () => {
    ...
    try {
      ...
      const [bills, purchases] = await Promise.all([fetchBills(), fetchPurchases()]);
      ...
      if (Object.keys(openingBalances).length) setOpeningBalances(openingBalances);
  ...
  }, [setStaff, setItems, setStock, setCustomers, setBills, setPurchases, setTransactions, setOpeningBalances]);
  ...
  // ── Login handler ─────────────────────────────────────────
  const handleLogin = useCallback((u: ERPUser) => {
